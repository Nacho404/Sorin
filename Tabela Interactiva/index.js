            ...
            if(i == n.length-2) {
                document.querySelector("." + trClass + " td").onmouseover = function () {
                    for(let j=1;j<m.length-1;j++){
                        m[j].classList.add("removeLine");
                    }
                }
        
                document.querySelector("." + trClass + " td").onmouseout = function () {
                    for(let j=1;j<m.length-1;j++){
                        m[j].classList.remove("removeLine");
                    }
                }
            } else {
                document.querySelector("." + trClass + " td").onmouseover = function () {
    ...
    for(let i=1; i<removeLine.length; i++){
        ...
        removeLine[i].onmouseover = function () {
            ...
            for(let k=1; k<n.length-1; k++) {
                let trClass =  "line-"+k;
                document.querySelector("." + trClass + " td:nth-of-type("+ countChild + ")").classList.add("removeColumn");
            }
        ...
        removeLine[i].onmouseout = function () {
            ...
            for(let k=1; k<n.length-1; k++) {
                let trClass =  "line-"+k;
                document.querySelector("." + trClass + " td:nth-of-type("+ countChild + ")").classList.remove("removeColumn");
            }
